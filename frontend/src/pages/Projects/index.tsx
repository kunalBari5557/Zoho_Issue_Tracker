...
const ProjectList = () =>{
    ...
    const projects = useSelector((state: any) => state?.projectsState?.projects.Project);
    const [project, setProjects] = useState<any>(projects);

    const rolesUser = useSelector((state: any) => state.roles?.roles);
    const totalPages = useSelector((state: any) => state.roles?.roles?.totalPages);
    ...
    const HandleDelete = (e:any) => {
        ...
        Swal.fire({
          ...
          confirmButtonText: 'Delete',
            ...
            if(result.isConfirmed===true){
                axios.delete(`${process.env.REACT_APP_URL}/admin/projects/delete/${e}`, {headers: {'token':`${localStorage.getItem('Token')}`} })
                .then((res)=>{
                   if (res.status===200) {
                       Swal.fire({
      ...
      const HandleEdit=(e:any)=>{
        Navigate("/admin/projects/edit",{state:e});
      }
        ...
        <div className="breadcrumb-two mb-3 d-flex justify-content-between" >
            ...
            <button className='btn btn-primary btn-rounded py-0'>
                <div className='d-flex'>
                    ...
                    <h5 className='text-white mb-0'onClick={()=>Navigate('/admin/projects/store')}>Add</h5>
                </div>
            </button>
        </div>
                        ...
                        <div className="row">
                            <div className="col-sm-12">
                                <table id="default-ordering" className="table table-hover dataTable" style={{width:"100%"}} role="grid" aria-describedby="default-ordering_info">
                                    <thead>
                                        <tr role="row">
                                            <th className="sorting col-2" tabIndex={0} rowSpan={1} colSpan={1}>Project Name</th>
                                            {/* <th className="sorting col-3" tabIndex={0} rowSpan={1} colSpan={1} style={{textAlign:'center'}}>Status</th> */}
                                            <th className="sorting col-3" tabIndex={0} rowSpan={1} colSpan={1} style={{textAlign:'center'}}>Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    {project?.length!==0?<>
