  ...
  const renderedIssues = openIssues?.map((issue: any) => {
    ...
    if (project) {
      return {
        ...
        issues: issue.issues,
        status_id: issue.status_id === 1 ? "Open" : null,
      };
    }
...
export const Resolved = () => {
  ...
  const issuesUser = useSelector((state: any) => state?.issue?.issues);
  const resolvedIssues = issuesUser.filter((item: any) => item?.status_id === 3);

  const renderedIssues = resolvedIssues?.map((issue: any) => {
    const project = projects?.find(
      (project: any) => project.id === issue.project_id
    );
    ...
    if (project) {
      return {
        ...
        projectName: project.projects,
        issues: issue.issues,
        status_id: issue.status_id === 3 ? "Resolved" : null,
      };
    }
